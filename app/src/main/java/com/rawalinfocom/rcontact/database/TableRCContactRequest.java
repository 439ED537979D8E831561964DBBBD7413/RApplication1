...
public class TableRCContactRequest {
    ...
    public int addRequestFromService(int status, String carId, String carMongodbRecordIndex, int carPmIdFrom, String requestType, String createdAt, String updatedAt
            , String name, String profilePhoto) {
        SQLiteDatabase db = databaseHandler.getWritableDatabase();
        boolean isDelete;

        if (requestType.equalsIgnoreCase("request all")) {
            isDelete = deleteRequestAllResponse(String.valueOf(carPmIdFrom), createdAt);
            System.out.println("RContacts delete request all --> " + isDelete);
        }
        ContentValues values = new ContentValues();
        values.put(COLUMN_CAR_STATUS, status);
    ...
    private boolean deleteRequestAllResponse(String rcpID, String createdAt) {

        boolean isDelete;
        SQLiteDatabase db = databaseHandler.getWritableDatabase();

        isDelete = db.delete(TABLE_RC_CONTACT_ACCESS_REQUEST, COLUMN_CAR_CREATED_AT + " = '" + createdAt + "' and " +
                COLUMN_CRM_RC_PROFILE_MASTER_PM_ID + " = '" + rcpID + "'", null) > 0;
