...
//        theta += Math.PI/2.0;
     ...
     */
    public void setBarStartEndLine(int _barWidth, BarStartEndLine _barStartEndLine, @ColorInt int
            _lineColor, float _sweepWidth) {
    ...
    public void setClippingBitmap(Bitmap _clippingBitmap) {
        ...
        if (getWidth() > 0 && getHeight() > 0) {
            mClippingBitmap = Bitmap.createScaledBitmap(_clippingBitmap, getWidth(), getHeight(),
                    false);
        } else {
    ...
    public void setOnAnimationStateChangedListener(AnimationStateChangedListener
                                                           _animationStateChangedListener) {
     ...
     *                           Only useful for autotextsize=true; Effects both, the unit text
    ...
    private void parseAttributes(TypedArray a) {
        ...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_barColor) && a.hasValue(R.styleable
                .CircleProgressView_cpv_barColor1) && a.hasValue(R.styleable
                .CircleProgressView_cpv_barColor2) && a.hasValue(R.styleable
                .CircleProgressView_cpv_barColor3)) {
            mBarColors = new int[]{a.getColor(R.styleable.CircleProgressView_cpv_barColor,
                    mBarColorStandard), a.getColor(R.styleable.CircleProgressView_cpv_barColor1,
                    mBarColorStandard), a.getColor(R.styleable.CircleProgressView_cpv_barColor2,
                    mBarColorStandard), a.getColor(R.styleable.CircleProgressView_cpv_barColor3,
                    mBarColorStandard)};

        } else if (a.hasValue(R.styleable.CircleProgressView_cpv_barColor) && a.hasValue(R
                .styleable.CircleProgressView_cpv_barColor1) && a.hasValue(R.styleable
                .CircleProgressView_cpv_barColor2)) {
            ...
            mBarColors = new int[]{a.getColor(R.styleable.CircleProgressView_cpv_barColor,
                ...
                .styleable.CircleProgressView_cpv_barColor1)) {
        ...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_barStrokeCap)) {
            setBarStrokeCap(StrokeCap.values()[a.getInt(R.styleable
                    .CircleProgressView_cpv_barStrokeCap, 0)].paintCap);
        }

        if (a.hasValue(R.styleable.CircleProgressView_cpv_barStartEndLineWidth) && a.hasValue(R
                .styleable.CircleProgressView_cpv_barStartEndLine)) {
            setBarStartEndLine((int) a.getDimension(R.styleable
                            .CircleProgressView_cpv_barStartEndLineWidth, 0),
                    BarStartEndLine.values()[a.getInt(R.styleable
                            .CircleProgressView_cpv_barStartEndLine, 3)],
                    a.getColor(R.styleable.CircleProgressView_cpv_barStartEndLineColor,
                            mBarStartEndLineColor),
                    a.getFloat(R.styleable.CircleProgressView_cpv_barStartEndLineSweep,
        ...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_unitPosition)) {
            setUnitPosition(UnitPosition.values()[a.getInt(R.styleable
                    .CircleProgressView_cpv_unitPosition, 3)]);
        }
        ...
        setShowTextWhileSpinning(a.getBoolean(R.styleable
                .CircleProgressView_cpv_showTextInSpinningMode, mShowTextWhileSpinning));
        ...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_textTypeface)) {
            try {
                textTypeface = Typeface.createFromAsset(getContext().getAssets(), a.getString(R
                        .styleable.CircleProgressView_cpv_textTypeface));
        ...
        if (a.hasValue(R.styleable.CircleProgressView_cpv_unitTypeface)) {
            try {
                unitTextTypeface = Typeface.createFromAsset(getContext().getAssets(), a.getString
                        (R.styleable.CircleProgressView_cpv_unitTypeface));
            } catch (Exception exception) {
                // error while trying to inflate typeface (is the path set correctly?)
            }
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (mClippingBitmap != null) {
            mClippingBitmap = Bitmap.createScaledBitmap(mClippingBitmap, getWidth(), getHeight(),
                    false);
        }
    ...
    private RectF getInnerCircleRect(RectF _circleBounds) {
        ...
        double circleWidth = +_circleBounds.width() - (Math.max(mBarWidth, mRimWidth)) -
                mOuterContourSize - mInnerContourSize;
        ...
        if (isUnitVisible()) {
            switch (mUnitPosition) {
                case TOP:
                case BOTTOM:
                    scaleX = 1.1f; // scaleX square to rectangle, so the longer text with unit
                    // fits better
                    scaleY = 0.88f;
                    break;
                case LEFT_TOP:
                ...
                case LEFT_BOTTOM:
                case RIGHT_BOTTOM:
                    scaleX = 0.77f; // scaleX square to rectangle, so the longer text with unit
                    // fits better
                    scaleY = 1.33f;
                    break;
            }
        ...
        return new RectF(_circleBounds.left + (widthDelta * scaleX), _circleBounds.top +
                (widthDelta * scaleY), _circleBounds.right - (widthDelta * scaleX), _circleBounds
                .bottom - (widthDelta * scaleY));
    ...
    private void setTextSizeAndTextBoundsWithAutoTextSize(float unitGapWidthHalf, float
            unitWidth, float unitGapHeightHalf, float unitHeight, String text) {
            ...
            switch (mUnitPosition) {
                ...
                case BOTTOM:
                    textRect = new RectF(mOuterTextBounds.left, mOuterTextBounds.top,
                            mOuterTextBounds.right, mOuterTextBounds.bottom - unitHeight -
                            unitGapHeightHalf);
                    break;
                case LEFT_TOP:
                case LEFT_BOTTOM:
                    textRect = new RectF(mOuterTextBounds.left + unitWidth + unitGapWidthHalf,
                            mOuterTextBounds.top, mOuterTextBounds.right, mOuterTextBounds.bottom);
                    break;
                case RIGHT_TOP:
                case RIGHT_BOTTOM:
                default:
    ...
    private void setUnitTextBoundsAndSizeWithAutoTextSize(float unitGapWidthHalf, float
            unitWidth, float unitGapHeightHalf, float unitHeight) {
        //calc the rectangle containing the unit text
        switch (mUnitPosition) {

            case TOP: {
                mUnitBounds = new RectF(mOuterTextBounds.left, mOuterTextBounds.top,
                        mOuterTextBounds.right, mOuterTextBounds.top + unitHeight -
                        unitGapHeightHalf);
                break;
            }
            case BOTTOM:
                mUnitBounds = new RectF(mOuterTextBounds.left, mOuterTextBounds.bottom -
                        unitHeight + unitGapHeightHalf, mOuterTextBounds.right, mOuterTextBounds
                        .bottom);
                break;
            case LEFT_TOP:
            case LEFT_BOTTOM: {
                mUnitBounds = new RectF(mOuterTextBounds.left, mOuterTextBounds.top,
                        mOuterTextBounds.left + unitWidth - unitGapWidthHalf, mOuterTextBounds
                        .top + unitHeight);
                break;
            }
            case RIGHT_TOP:
            case RIGHT_BOTTOM:
            default: {
                mUnitBounds = new RectF(mOuterTextBounds.right - unitWidth + unitGapWidthHalf,
                        mOuterTextBounds.top, mOuterTextBounds.right, mOuterTextBounds.top +
                        unitHeight);
            }
            break;
        }

        mUnitTextPaint.setTextSize(calcTextSizeForRect(mUnit, mUnitTextPaint, mUnitBounds) *
                mUnitScale);
        mUnitBounds = calcTextBounds(mUnit, mUnitTextPaint, mUnitBounds); // center text in
        // rectangle and reuse it
    ...
    private void setUnitTextBoundsAndSizeWithFixedTextSize(float unitGapWidth, float
            unitGapHeight) {
        mUnitTextPaint.setTextSize(mUnitTextSize);
        mUnitBounds = calcTextBounds(mUnit, mUnitTextPaint, mOuterTextBounds); // center text in
        // rectangle and reuse it

        switch (mUnitPosition) {

            case TOP:
                mUnitBounds.offsetTo(mUnitBounds.left, mActualTextBounds.top - unitGapHeight -
                        mUnitBounds.height());
                break;
            case BOTTOM:
            ...
            case LEFT_TOP:
            case LEFT_BOTTOM:
                mUnitBounds.offsetTo(mActualTextBounds.left - unitGapWidth - mUnitBounds.width(),
                        mUnitBounds.top);
                break;
            case RIGHT_TOP:
            case RIGHT_BOTTOM:
    ...
    private RectF calcTextBounds(String _text, Paint _textPaint, RectF _textBounds) {
        ...
        float height = textBoundsTmp.bottom + textBoundsTmp.height() * 0.93f; // the height of
        // calcTextBounds is a bit to high, therefore  * 0.93
        //center in circle
    ...
    private void setupBounds() {
        ...
        float circleWidthHalf = mBarWidth / 2f > mRimWidth / 2f + mOuterContourSize ? mBarWidth /
                2f : mRimWidth / 2f + mOuterContourSize;
        ...
        mCircleInnerContour = new RectF(mCircleBounds.left + (mRimWidth / 2.0f) +
                (mInnerContourSize / 2.0f), mCircleBounds.top + (mRimWidth / 2.0f) +
                (mInnerContourSize / 2.0f), mCircleBounds.right - (mRimWidth / 2.0f) -
                (mInnerContourSize / 2.0f), mCircleBounds.bottom - (mRimWidth / 2.0f) -
                (mInnerContourSize / 2.0f));
        mCircleOuterContour = new RectF(mCircleBounds.left - (mRimWidth / 2.0f) -
                (mOuterContourSize / 2.0f), mCircleBounds.top - (mRimWidth / 2.0f) -
                (mOuterContourSize / 2.0f), mCircleBounds.right + (mRimWidth / 2.0f) +
                (mOuterContourSize / 2.0f), mCircleBounds.bottom + (mRimWidth / 2.0f) +
                (mOuterContourSize / 2.0f));
    ...
    private void setupBarPaint() {
        if (mBarColors.length > 1) {
            mBarPaint.setShader(new SweepGradient(mCircleBounds.centerX(), mCircleBounds.centerY
                    (), mBarColors, null));
            Matrix matrix = new Matrix();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Draw spinner
        if (mAnimationState == AnimationState.SPINNING || mAnimationState == AnimationState
                .END_SPINNING) {
            drawSpinner(canvas);
    ...
    private void drawStartEndLine(Canvas _canvas, float _degrees) {
        ...
        if (mBarStartEndLine == BarStartEndLine.END || mBarStartEndLine == BarStartEndLine.BOTH) {
            _canvas.drawArc(mCircleBounds, startAngle + _degrees, mBarStartEndLineSweep, false,
                    mBarStartEndLinePaint);
        }
    }
    ...
    private void drawBlocks(Canvas _canvas, RectF circleBounds, float startAngle, float _degrees,
        ...
        while (tmpDegree < _degrees) {
            _canvas.drawArc(circleBounds, startAngle + tmpDegree, Math.min(mBlockScaleDegree,
                    _degrees - tmpDegree), userCenter, paint);
    ...
    private void drawTextWithUnit(Canvas canvas) {
        ...
        if (mTextLength != text.length()) {
            ...
            if (mTextLength == 1) {
                mOuterTextBounds = getInnerCircleRect(mCircleBounds);
                mOuterTextBounds = new RectF(mOuterTextBounds.left + (mOuterTextBounds.width() *
                        0.1f), mOuterTextBounds.top, mOuterTextBounds.right - (mOuterTextBounds
        ...
        canvas.drawText(text, mActualTextBounds.left - (mTextPaint.getTextSize() * 0.02f),
                mActualTextBounds.bottom, mTextPaint);
        ...
        if (mShowUnit) {
            ...
            if (update) {
                ...
                if (mIsAutoTextSize) {
                    ...
                    setUnitTextBoundsAndSizeWithFixedTextSize(unitGapWidthHalf * 2f,
            ...
            canvas.drawText(mUnit, mUnitBounds.left - (mUnitTextPaint.getTextSize() * 0.02f),
                    mUnitBounds.bottom, mUnitTextPaint);
        }
    }
        ...
        if (!mShowBlock) {
            ...
            if (mBarStrokeCap != Paint.Cap.BUTT && _degrees > 0 && mBarColors.length > 1) {
                if (_degrees > 180) {
                    ...
                    _canvas.drawArc(mCircleBounds, startAngle + (_degrees / 2), _degrees / 2,
                            false, mBarPaint);
                } else {
