...
/**
 * Created by Monal on 19/10/16.
 * <p>
 * A Class to create ripple effect for the views
 */
...
public class RippleView extends RelativeLayout {
    ...
    private void init(final Context context, final AttributeSet attrs) {
        ...
        rippleColor = typedArray.getColor(R.styleable.RippleView_rv_color, ContextCompat.getColor
    ...
    public void draw(Canvas canvas) {
        ...
        if (animationRunning) {
            ...
            if (rippleDuration <= timer * frameRate) {
                ...
                durationEmpty = -1;
                timerEmpty = 0;
                // There is problem on Android M where canvas.restore() seems to be called
                // automatically
            ...
            if (rippleType == 1 && originBitmap != null && (((float) timer * frameRate) /
                    rippleDuration) > 0.4f) {
                if (durationEmpty == -1)
                    durationEmpty = rippleDuration - timer * frameRate;

                timerEmpty++;
                final Bitmap tmpBitmap = getCircleBitmap((int) ((radiusMax) * (((float)
                        timerEmpty * frameRate) / (durationEmpty))));
            ...
            if (rippleType == 1) {
                if ((((float) timer * frameRate) / rippleDuration) > 0.6f)
                    paint.setAlpha((int) (rippleAlpha - ((rippleAlpha) * (((float) timerEmpty *
                            frameRate) / (durationEmpty)))));
                else
                    paint.setAlpha(rippleAlpha);
            } else
