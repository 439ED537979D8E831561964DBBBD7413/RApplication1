...
public class RecyclerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    /**
     * Create a decoration that draws a line in the given color and width between the items in
     * the view.
     *
     ...
     */
    public RecyclerItemDecoration(@NonNull Context context, @ColorInt int color,
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        // we set the stroke width before, so as to correctly draw the line we have to offset by
        // width / 2
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (position < state.getItemCount()) {
                c.drawLine(view.getLeft(), view.getBottom() + offset, view.getRight(), view
                        .getBottom() + offset, mPaint);
            }
        }
    }
}
